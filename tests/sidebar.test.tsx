...
it("renders the filesystem root without changing trailing-separator compaction", () => {
	...
	expect(
		groups.slice(1, 3).every((group) => stringWidth(group.fact) <= 28),
	).toBe(true);
});

it("retains complete compact paths for the renderer to wrap", () => {
	const groups = buildSidebarGroups(
		snapshot({
	...
	expect(groups[1].fact).toBe("very-long-parent-name/very-long-worktree-name");
	expect(groups[2].fact).toBe(
		"another-long-parent-name/very-long-working-tree-name",
	);
});
	...
	[
		{
			value: {
				...
				message: "GitHub unavailable" as const,
			},
			stale: false,
			status: "ready" as const,
		},
...
it("omits zero working-tree counts and renders clean when all counts are zero", () => {
	...
	expect(buildSidebarGroups(snapshot({ modified: 3 }), 28)[3].fact).toBe(
		"3 modified",
	);
});
...
it.each([
	...
	"shortens working-tree labels in order to fit %i columns",
	(width, expected) => {
		expect(
			buildSidebarGroups(
				snapshot({ staged: 2, modified: 3, untracked: 1 }),
				width,
			)[3].fact,
		).toBe(expected);
	},
...
it("retains all CJK characters for the renderer to wrap", () => {
	const fact = buildSidebarGroups(snapshot({ branch: "界".repeat(20) }), 28)[0]
		.fact;

	expect(fact).toBe("界".repeat(20));
});

it("retains complete emoji graphemes for the renderer to wrap", () => {
	const emoji = "👩‍💻";
	const fact = buildSidebarGroups(snapshot({ branch: emoji.repeat(20) }), 28)[0]
		.fact;

	expect(fact).toBe(emoji.repeat(20));
});
...
it("reads the current snapshot while rendering and hides an empty group list", () => {
	const readSnapshot = vi.fn(() => ({
		local: {
			value: { repository: false as const },
			stale: false,
			status: "ready" as const,
		},
		remote: { value: null, stale: false, status: "loading" as const },
	}));
	const renderSidebar = sidebarSlot({} as TuiPluginApi, readSnapshot, vi.fn())
...
it("renders the approved vertical stack with themed rows and ASCII dividers", () => {
	const renderSidebar = sidebarSlot({} as TuiPluginApi, snapshot, vi.fn()).slots
		.sidebar_content;
	const view = renderSidebar?.(slotContext, {
		session_id: "session",
	...
	const row = (text: string) =>
		rows.find((element) => textOf(element) === text);
...
it("mutes all stale local and pull-request facts", () => {
	...
	const renderSidebar = sidebarSlot(
		{} as TuiPluginApi,
		() => staleSnapshot,
		vi.fn(),
	).slots.sidebar_content;
...
it("invokes refresh from the header control", () => {
	...
	const renderSidebar = sidebarSlot({} as TuiPluginApi, snapshot, onRefresh)
		.slots.sidebar_content;
	const view = renderSidebar?.(slotContext, {
		session_id: "session",
	}) as unknown as TestElement;
