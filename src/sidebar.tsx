...
function compactPath(value: string): string {
	...
	return parent ? `${parent}/${name}` : name || value;
}
...
function workingTreeFact(
	...
	if (state.staged === 0 && state.modified === 0 && state.untracked === 0)
		return "clean";
	...
	return facts.find((fact) => stringWidth(fact) <= width) ?? facts.at(-1) ?? "";
}
...
function pullRequestFact(snapshot: RefreshSnapshot, width: number): string {
	...
	if (remote.kind === "unavailable") return remote.message;
...
function pullRequestTone(snapshot: RefreshSnapshot): SidebarGroup["tone"] {
	const remote = snapshot.remote.value;
	if (snapshot.remote.status !== "ready" || !remote || remote.kind !== "ready")
		return "muted";
	...
	return (
		...
		>
			<box flexDirection="row" justifyContent="space-between" width="100%">
				...
				{/* biome-ignore lint/a11y/noStaticElementInteractions: OpenTUI terminal mouse target; keyboard refresh is registered in the command palette. */}
